#!/usr/bin/env python3
"""
Fix non-English tool pages - V6: Simple, robust, handles both patterns.
REPLACE: nav-v2-style block + hero-atmo-style block + <nav id="nav-v2">...</nav> block.
PRESERVE: Everything else - hero section, calculator, SEO content, footer.
"""
...
def fix_file(filepath):
    """Replace style blocks and nav HTML. Never touch content."""
    ...
    # ── STEP 1: Replace <style id="nav-v2-style"> block ──
    s_start = '<style id="nav-v2-style">'
    idx1 = content.find(s_start)
    if idx1 == -1:
        return False, "no nav-v2-style"
    
    # Find matching </style>: the one right before <style id="hero-atmo-style">
    idx_atmo = content.find('<style id="hero-atmo-style">', idx1)
    if idx_atmo == -1:
        return False, "no hero-atmo-style anchor"
    
    # Find the </style> that closes nav-v2-style: it's the one just before hero-atmo-style
    end1 = content.rfind('</style>', idx1, idx_atmo)
    if end1 == -1:
        return False, "no close for nav-v2-style"
    end1 += len('</style>')
    
    content = content[:idx1] + NAV_V2_STYLE + content[end1:]
    changes.append("nav-v2-style")
    
    # ── STEP 2: Replace <style id="hero-atmo-style"> block ──
    # Recalculate
    idx_atmo2 = content.find('<style id="hero-atmo-style">')
    if idx_atmo2 == -1:
        return False, "lost hero-atmo-style after step 1"
    
    # Find the </style> that closes it: the one right before <style id="hero-img-style">
    idx_img = content.find('<style id="hero-img-style">', idx_atmo2)
    if idx_img == -1:
        return False, "no hero-img-style anchor"
    
    end2 = content.rfind('</style>', idx_atmo2, idx_img)
    if end2 == -1:
        return False, "no close for hero-atmo-style"
    end2 += len('</style>')
    
    content = content[:idx_atmo2] + HERO_ATMO_STYLE + content[end2:]
    changes.append("hero-atmo-style")
    
    # ── STEP 3: Replace <nav id="nav-v2">...</nav> block ──
    nav_tag = '<nav id="nav-v2">'
    nav_pos = content.find(nav_tag)
    if nav_pos == -1:
        return False, "no nav tag"
    
    # Find matching </nav> — naive but works since there's only one nav in these pages
    nav_close = content.find('</nav>', nav_pos + len(nav_tag))
    if nav_close == -1:
        return False, "no nav close tag"
    nav_close += len('</nav>')
    
    new_nav = build_nav_html(lang, tool_path)
    content = content[:nav_pos] + new_nav + content[nav_close:]
    changes.append("nav-html")
    
    # ── STEP 4: Inject toggleLangDD JS before </body> ──
    ...
    # ── STEP 5: Remove ANY duplicate <nav id="nav-v2"> blocks ──
    nav_count = content.count('<nav id="nav-v2">')
    if nav_count > 1:
        # Keep only the FIRST nav block, remove all later ones
        first_nav = content.find('<nav id="nav-v2">')
        # Find and remove subsequent nav blocks
        while True:
            next_nav = content.find('<nav id="nav-v2">', first_nav + 1)
            if next_nav == -1:
                break
            next_close = content.find('</nav>', next_nav + len(nav_tag))
            if next_close == -1:
                break
            next_close += len('</nav>')
            content = content[:next_nav] + content[next_close:]
            if 'removed-dup-nav' not in changes:
                changes.append("removed-dup-nav")
    
    # ── STEP 6: Clean up orphaned CSS fragments ──
    # Remove duplicate engineering-directory CSS blocks
    eng_css_count = content.count('.engineering-directory a{')
    if eng_css_count > 1:
        first = content.find('.engineering-directory a{')
        second = content.find('.engineering-directory a{', first + 1)
        while second != -1:
            # Find the <style> block containing this duplicate
            block_start = content.rfind('<style>', 0, second)
            if block_start != -1:
                block_end = content.find('</style>', second)
                if block_end != -1:
                    block_end += len('</style>')
...
def verify_file(filepath):
    ...
    issues = []
    if 'class="hero"' not in content and '<section class="hero">' not in content:
        issues.append("MISSING hero section")
    if 'id="calculator"' not in content:
        issues.append("MISSING calculator")
    if 'b2b-card' not in content:
        issues.append("MISSING b2b-card")
    if 'function toggleLangDD' not in content:
        issues.append("MISSING toggleLangDD JS")
    if content.count('<nav id="nav-v2">') != 1:
        issues.append(f"WRONG nav count: {content.count('<nav id="nav-v2">')}")
...
def main():
            ...
            try:
                ...
                if ok:
                    ...
                    if issues:
                        ...
                        verify_failures.append((fp, issues))
                else:
                    errors += 1
    ...
    if verify_failures:
        print(f"\n⚠️  Verification issues ({len(verify_failures)}):")
        for vf, issues in verify_failures:
            print(f"  - {os.path.relpath(vf, ROOT)}: {', '.join(issues)}")
