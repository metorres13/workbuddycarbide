...
def build_lang_dropdown(current_lang, tool_path):
    ...
    # Derive the tool path suffix (e.g. /tools/speed-feed/)
    parts = []
    for d in ALL_LANGS:
        code = d[0]
        if code == "en":
            url = "/" + tool_path.lstrip("/") if not tool_path.startswith("/" + code) else tool_path
            # Normalize: ensure the URL path matches
            if current_lang == "en":
                href = tool_path
            else:
                # Replace current lang prefix with en prefix
                href = tool_path.replace(f"/{current_lang}/", "/", 1) if current_lang != "en" else tool_path
        else:
            if current_lang == "en":
                href = "/" + code + tool_path
            else:
                href = tool_path.replace(f"/{current_lang}/", f"/{code}/", 1)
